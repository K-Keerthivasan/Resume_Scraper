// ==UserScript==
// @name         London Tech Jobs Saver + Local Collector
// @namespace    https://k2digitalmedia.ca
// @version      1.0
// @description  Saves LondonTechJobs.ca postings into the local Resume_Scraper app.
// @author       Keerthi (K2 Digital Media)
// @match        https://londontechjobs.ca/*
// @match        https://www.londontechjobs.ca/*
...
(function () {
  ...
  function getJsonLdJobPosting() {
    ...
    for (const script of scripts) {
      try {
        ...
        // Ignore malformed metadata and continue with DOM scraping.
      }
    }
    return {};
  }
  ...
  function cleanTitle(value) {
    return cleanText(value).replace(/^NEW\s+/i, "").replace(/\s+-\s+London Tech Jobs.*$/i, "");
  }

  function isNavHeading(value) {
    return /^(employers|posting list|search|find a job|post a job|login|register|about|contact us)$/i.test(cleanText(value));
  }

  function getHeadingTexts() {
    return [...document.querySelectorAll("h1, h2, h3, h4")]
      .map((el) => cleanText(el.innerText || el.textContent || ""))
      .filter((text) => text && !isNavHeading(text));
  }

  function firstLineMatching(pattern, afterText = "") {
    const lines = getPageLines();
    const start = afterText ? Math.max(0, lines.findIndex((line) => line.includes(afterText))) : 0;
    return lines.slice(start).find((line) => pattern.test(line)) || "";
  }
  ...
  function getJobId() {
    try {
      ...
      return parsed.searchParams.get("jid") || parsed.searchParams.get("jobid") || parsed.searchParams.get("id") || "";
    } catch (error) {
      return "";
    }
  }
  ...
  function isJobPage() {
    ...
    if (/joblist\.aspx|joblistexport\.aspx/.test(path)) return false;
    if (getJobId()) return true;
    if (/job_print\.aspx|job\.aspx|jobs\/.+/.test(path)) return true;
    return Boolean(getJsonLdJobPosting()?.title);
  }

  function getApplyUrl() {
    const applyLink = [...document.querySelectorAll("a[href]")]
      .map((link) => link.href)
      .find((href) => /apply|career|careers|jobs|mailto:/i.test(href) && !/londontechjobs\.ca\/joblist/i.test(href));
    return applyLink || window.location.href;
  }
  ...
  function getKeySkills(description) {
    const listItems = [...document.querySelectorAll("main li, article li, .job li, li")]
      .map((li) => cleanText(li.innerText || li.textContent || ""))
      .filter((text) => text.length > 5 && text.length < 140 && !/login|register|find a job|post a job/i.test(text))
      .slice(0, 8);
  ...
  function getDescription() {
    const text =
      readText([
        ...
        "article",
        "main",
      ]) || getMeta("description") || document.body.innerText;

    return cleanText(text).slice(0, 800) || "N/A";
  }

  function scrapeCurrentJob() {
    const jsonLd = getJsonLdJobPosting();
    const headings = getHeadingTexts();
    const title = cleanTitle(jsonLd.title || getMeta("og:title") || headings[0] || document.title || "Unknown Title");
    const titleIndex = headings.findIndex((heading) => cleanTitle(heading) === title);
    const companyHeading = headings.slice(Math.max(0, titleIndex + 1)).find((heading) => !/new\s+/i.test(heading)) || "";
    ...
    const company = cleanText(jsonLd.hiringOrganization?.name) || companyHeading || "Unknown Company";
    const location =
      getJsonLdLocation(jsonLd) ||
      firstLineMatching(/\b(remote|london|st\.?\s*thomas|ontario|on|canada)\b/i, title) ||
      "Unknown Location";
    const postedDate =
      cleanText(jsonLd.datePosted) ||
      firstLineMatching(/\b(jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)[a-z]*\.?\s+\d{1,2},\s+\d{4}\b/i, title) ||
      "N/A";
    ...
    return {
      ...
      company,
      location,
      job_type: cleanText(jsonLd.employmentType) || firstLineMatching(/full.time|part.time|contract|permanent|temporary|intern|co-op|casual/i, title) || "N/A",
      salary: getJsonLdSalary(jsonLd) || firstLineMatching(/\$|salary|hour|year|wage/i, title) || "N/A",
  ...
  function showToast(message, duration = 3200) {
    const existing = document.getElementById("tm-ltj-toast");
    if (existing) existing.remove();

    const toast = document.createElement("div");
    toast.id = "tm-ltj-toast";
  ...
  function hideDuplicateModal() {
    document.getElementById("tm-ltj-duplicate-modal")?.remove();
  }
  ...
  function showDuplicateModal(job, duplicateInfo) {
    ...
    overlay.id = "tm-ltj-duplicate-modal";
      ...
      <div style="width:min(520px, 100%);background:linear-gradient(180deg, rgba(12,20,35,0.98), rgba(7,13,24,0.98));color:#eef6ff;border-radius:22px;box-shadow:0 28px 70px rgba(0,0,0,0.42);border:1px solid rgba(106,194,255,0.16);overflow:hidden;font-family:'Segoe UI',sans-serif;">
        ...
        <div style="padding:20px 22px;display:grid;gap:14px;">
          ...
          <div style="display:flex;gap:10px;justify-content:flex-end;flex-wrap:wrap;">
            <button id="tm-ltj-dup-cancel" type="button" style="padding:11px 14px;border-radius:12px;border:1px solid rgba(106,194,255,0.14);background:rgba(255,255,255,0.03);color:#eef6ff;cursor:pointer;font:inherit;">Cancel</button>
            <button id="tm-ltj-dup-save" type="button" style="padding:11px 14px;border-radius:12px;border:0;background:linear-gradient(135deg,#20b8f0,#45d0ff);color:#04111a;cursor:pointer;font:inherit;font-weight:700;">Save second entry</button>
          </div>
        </div>
      </div>
    `;
    ...
    document.getElementById("tm-ltj-dup-cancel")?.addEventListener("click", hideDuplicateModal);
    document.getElementById("tm-ltj-dup-save")?.addEventListener("click", async () => {
      hideDuplicateModal();
  ...
  async function saveJobToLocalApp(allowDuplicate = false) {
    if (!isJobPage()) {
      showToast("Open a London Tech Jobs posting before saving.");
      return;
    }
  ...
  function renderPanel() {
    const titleNode = document.getElementById("tm-ltj-job-title");
    const metaNode = document.getElementById("tm-ltj-job-meta");
    const routeNode = document.getElementById("tm-ltj-job-route");
    const emailNode = document.getElementById("tm-ltj-job-email");
    const skillsNode = document.getElementById("tm-ltj-job-skills");
    const skillsWrap = document.getElementById("tm-ltj-job-skills-wrap");
    const toggleSkillsBtn = document.getElementById("tm-ltj-toggle-skills-btn");
    const statusNode = document.getElementById("tm-ltj-collector-status");
    const saveBtn = document.getElementById("tm-ltj-save-btn");
    ...
    if (preview) {
      ...
      metaNode.textContent = "The saver stays inactive on the listing page to avoid saving the wrong job.";
      routeNode.textContent = "Open a posting URL with a job ID, then save.";
  ...
  function createPanel() {
    if (document.getElementById("tm-ltj-panel")) return;

    const panel = document.createElement("div");
    panel.id = "tm-ltj-panel";
    ...
    panel.innerHTML = `
      <div style="padding:18px 18px 14px;background:linear-gradient(135deg,#071221 0%,#102746 100%);color:#eef6ff;">
        <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:12px;">
          <div>
            <div style="font-size:11px;letter-spacing:0.14em;text-transform:uppercase;font-weight:700;color:#ff8b5e;">London Tech Jobs Saver</div>
            <div style="font-size:22px;font-weight:700;margin-top:8px;line-height:1.05;">Local Job Capture</div>
          </div>
          <button id="tm-ltj-min-btn" type="button" style="border:1px solid rgba(106,194,255,0.16);background:rgba(255,255,255,0.06);color:#eef6ff;width:32px;height:32px;border-radius:999px;cursor:pointer;font-size:18px;line-height:1;">-</button>
        </div>
        <div id="tm-ltj-collector-status" style="margin-top:14px;display:inline-flex;padding:7px 10px;border-radius:999px;font-size:12px;font-weight:700;">Collector offline</div>
      </div>

      <div id="tm-ltj-panel-body" style="padding:16px;display:grid;gap:14px;">
        <div style="padding:14px;border-radius:18px;background:linear-gradient(180deg, rgba(255,255,255,0.03), rgba(255,255,255,0.01));border:1px solid rgba(106, 194, 255, 0.12);box-shadow:inset 0 1px 0 rgba(255,255,255,0.03);">
          ...
          <div id="tm-ltj-job-title" style="margin-top:8px;font-size:18px;line-height:1.18;font-weight:700;">Loading...</div>
          <div id="tm-ltj-job-meta" style="margin-top:6px;font-size:13px;color:#9eb2d1;line-height:1.45;"></div>
          <div id="tm-ltj-job-route" style="margin-top:8px;font-size:12px;color:#45d0ff;font-weight:700;"></div>
          <div id="tm-ltj-job-email" style="margin-top:6px;font-size:12px;line-height:1.5;color:#9eb2d1;"></div>
          <div style="margin-top:12px;display:flex;justify-content:space-between;align-items:center;gap:12px;">
            ...
            <button id="tm-ltj-toggle-skills-btn" type="button" style="padding:6px 10px;border-radius:999px;border:1px solid rgba(106,194,255,0.14);background:rgba(255,255,255,0.04);color:#eef6ff;cursor:pointer;font:inherit;font-size:11px;">Hide Skills</button>
          </div>
          <div id="tm-ltj-job-skills-wrap" style="display:block;">
            <div id="tm-ltj-job-skills" style="margin-top:8px;font-size:12px;line-height:1.55;color:#c8d7f0;"></div>
          </div>
        </div>

        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;">
          <button id="tm-ltj-save-btn" type="button" title="Save Job (Alt+Shift+S)" style="padding:12px 14px;border-radius:14px;border:0;background:linear-gradient(135deg,#20b8f0,#45d0ff);color:#04111a;cursor:pointer;font:inherit;font-weight:700;box-shadow:0 12px 28px rgba(32,184,240,0.22);">Save Job</button>
          <button id="tm-ltj-fill-btn" type="button" style="padding:12px 14px;border-radius:14px;border:1px solid rgba(106,194,255,0.14);background:rgba(255,255,255,0.04);color:#eef6ff;cursor:pointer;font:inherit;font-weight:700;">Auto Fill</button>
          <button id="tm-ltj-refresh-btn" type="button" style="padding:12px 14px;border-radius:14px;border:1px solid rgba(106,194,255,0.14);background:rgba(255,255,255,0.04);color:#eef6ff;cursor:pointer;font:inherit;">Refresh</button>
          <button id="tm-ltj-dashboard-btn" type="button" style="padding:12px 14px;border-radius:14px;border:1px solid rgba(106,194,255,0.14);background:rgba(255,255,255,0.04);color:#eef6ff;cursor:pointer;font:inherit;">Dashboard</button>
        </div>
    ...
    const body = panel.querySelector("#tm-ltj-panel-body");
    const minBtn = panel.querySelector("#tm-ltj-min-btn");
    let collapsed = false;
    ...
    panel.querySelector("#tm-ltj-save-btn")?.addEventListener("click", () => saveJobToLocalApp(false));
    panel.querySelector("#tm-ltj-fill-btn")?.addEventListener("click", autoFillForm);
    panel.querySelector("#tm-ltj-toggle-skills-btn")?.addEventListener("click", () => {
      state.showSkills = !state.showSkills;
      renderPanel();
    });
    panel.querySelector("#tm-ltj-refresh-btn")?.addEventListener("click", async () => {
      renderPanel();
      await pingCollector();
      showToast("Panel refreshed.");
    });
    panel.querySelector("#tm-ltj-dashboard-btn")?.addEventListener("click", () => {
